...
function addMsg(msgData){
  if (!msgData.user.image) {
    msgData.user.image = "/images/user.svg";
  }
	var msg = `
		<div class="message ${msgData.direction} mb-3" data-time="${msgData.time}">
			<div class="user">
				<img class="rounded" src="${msgData.user.image}" height="35" width="35">
        <strong class="mx-2">${msgData.user.name}:</strong>
        ${msgData.msg}
        <small class="text-muted ml-2 time">Just now</small>
			</div>
		</div>
	`;
	$('#chat').append(msg);
