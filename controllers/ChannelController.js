...
class ChannelController {
  ...
  fetchChannelsData = async (request, response, next) => {
    ...
    if (currentUser) {
      try {
        ...
        response.locals.channels.subscribed = await AraDTChannelModel.getSubscribedChannels(
          request
        );
        response.locals.channels.owned = await AraDTChannelModel.getOwnedChannels(
          request
        );
        console.log(
  ...
  fetchChannel = async (request, response, next) => {
    try {
      var channelId = request.params.channelId;
      if (channelId) {
        response.locals.channel = await AraDTChannelModel.fetchChannel(
          channelId
        );
  ...
  fetchMessages = async (request, response, next) => {
    try {
      ...
      if (channelId) {
        response.locals.messages = await AraDTChannelModel.fetchMessages(
          channelId,
          userId
        );
      }
