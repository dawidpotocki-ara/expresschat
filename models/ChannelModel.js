...
class ChannelModel {
  ...
  fetchChannel = async (channelId) => {
    ...
    var channelDoc = AraDTDatabase.storage
      .collection("channels")
      .doc(channelId);
    var channel = {};
    await channelDoc
  ...
  updateChannel = async (request, response) => {
    ...
    try {
      ...
      await channelDoc.update(updatedChannel).catch((error) => {
        throw Error(error);
      });
    } catch (error) {
      throw error;
  ...
  buildChannel = async (request) => {
    ...
    var image = "";
    var avatar =
      request.files && request.files.avatar ? request.files.avatar : false;
    ...
    if (avatar) {
      var { result, validExtension } = AraDTImageUpload.uploadImage(
        avatar,
        slugName
      );
      if (validExtension) {
    ...
    await channelDoc
      ...
      .then((datum) => {
        if (datum.data().owner == currentUserId) {
          ...
          throw new Error([
            "You do not have authorisation to delete this channel",
          ]);
        }
      })
      .catch(() => {
